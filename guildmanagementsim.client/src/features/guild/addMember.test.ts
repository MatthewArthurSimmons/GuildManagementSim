...
describe('addMemberToGuild', () => {
  it('adds a member to the guild', () => {
    ...
    const memberToBeAdded = createMember(guild.guildID, 'Ilona', 5, 'Idle')
  ...
  it('throws on duplicate member (by Name)', () => {
    const guild = createGuild('Guild', 'desc', 1, 10);
    const member1 = createMember(guild.guildID, 'Matt', 2, 'Idle');
    const member2 = createMember(guild.guildID, 'Matt', 5, 'Idle');
